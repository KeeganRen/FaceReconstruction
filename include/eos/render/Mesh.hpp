 ...
 * @param[in] filename Output filename.
 */
inline void write_textured_obj(Mesh mesh, std::string filename, std::string filepath)
{
	assert((mesh.vertices.size() == mesh.colors.size() || mesh.colors.empty()) && !mesh.texcoords.empty());

    std::ofstream obj_file(filepath+filename);
    ...
    std::ofstream mtl_file(filepath+mtl_filename.string());
